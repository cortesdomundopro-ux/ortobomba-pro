...
const ADMIN_CODE: string =
  (import.meta as Record<string, unknown> & { env: Record<string, string | undefined> }).env
...
(function initParticles() {
  ...
  function draw() {
    ...
    pts.forEach((p) => {
      ...
      ctx.fillStyle = `rgba(200,100,255,${op})`;
      ctx.fill();
    });
  }
  draw();
...
function renderQuestion() {
  ...
  el<HTMLDivElement>("q-word").innerHTML = esc(q.word).replaceAll("__", `<span class="q-blank">?</span>`);
